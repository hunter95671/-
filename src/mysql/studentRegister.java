...
public class studentRegister {
    public static void main(String[] args) {
        ...
        //下面是连接数据库查询学生用户表，查询是否存在于学生表中并且已经注册，如果存在于学生表中并且没有注册，则插入该用户
        ...
        try {
            ...
            String sql1="select * from luox_studentpsw;";
            String userpassword1= new String();

            boolean boolt=true;//用来标识是否停止循环
            boolean bool0=false;//用来标识学生是否存在于学生表
            while (boolt) {
                bool0=false;
                rs0=stmt.executeQuery(sql0);
                while (rs0.next()) {
                    String uname = rs0.getString("lx_Sno");
                ...
                if (bool0 == false) {//如果学生不存在则重新输入或退出
                    System.out.println("该学生不存在，请重新输入或返回上一级菜单！");
            ...
            //向学生用户表中插入数据
            String sql2="insert into  luox_studentpsw values (\'"+name+"\',\'"+userpassword1+"\');";
            int count=stmt.executeUpdate(sql2);
            System.out.println(count == 1 ? "学生用户注册成功！" : "学生用户注册失败！");
